...
- 2026-08-19 — Built every Sales-Rep problem (PR-1…7 from prd.txt §9.1) into the Deal detail page's "Deal Health · Survey Signals" panel (KIA-SEL-002). Added: a real-time negative-response alert banner (PR-2, "New response 2h ago… Notified: Jay (owner)"); a "Revenue at Risk · this deal" stat card showing $6,300 = $15,000 × (1 − 0.58 sentiment) (PR-3); a sentiment-trend mini-bar 84→58 ▼ (PR-7); a driving-signal + verbatim quote block surfacing the customer's own words "Price higher… delivery too long" (RV-3/RV-5); a stakeholder-coverage row "2 of 4 responded" with responded/silent chips = multi-threading risk (PR-4/R6); a Next Best Action card with an auto-created recovery Task assigned to Jay + due-24h, plus "Log recovery & clear flag" and "Call Ramesh" buttons (PR-5/R2/R3); and relabeled the touchpoints table caption to "auto-sent by workflow on CRM events (no manual sending)" (PR-6). Wired cvDealRecover() (loop-close: badge→Recovered, at-risk $6,300→$0, alert→resolved, note that the manager total drops by $6,300) and cvResetDealPanel() (called on openDealDetailPage so the panel resets each open). Why: user asked to implement every rep problem from the PRD into the mock as the rep-facing surface; verified live at localhost:8080 (alert/stat/trend/verbatim/coverage/NBA render, recovery button clears the flag).
